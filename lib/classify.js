...
const KEYWORDS = {
  ...
  streetlight: ['streetlight', 'street light', 'street-light', 'streetlights', 'lamp', 'bulb', 'light out', 'light band', 'اندھیرا', 'بتی', 'اسٹریٹ لائٹ', 'سٹریٹ لائٹ', 'روشنی'],
...
function ruleSeverity(category, text) {
  ...
  switch (category) {
    ...
    case 'streetlight': return /اندھیرا|dark|accident|حادثہ/.test(t) ? 'medium' : 'low';
    default: return 'low';
  }
}
...
export function fallbackClassify(rawText, city, area) {
  ...
  return {
    ...
    confidence,
    summary_en: `Reported ${CATEGORY_PHRASE[category].toLowerCase()} in ${place}${cityEn}.`,
    summary_ur: `${place}${cityUr} میں ${category === 'other' ? 'ایک شہری مسئلے' : CATEGORY_PHRASE_UR[category]} کی شکایت درج کروائی گئی ہے۔`,
...
Given a citizen's complaint (written in Urdu, Roman Urdu, English or a mix), a city and an area, plus a list of candidate government departments from a verified knowledge base, return STRICT JSON only, no markdown fences:
{
  ...
  "summary_en": max 25 words, factual one-sentence summary in English,
  "summary_ur": same summary in Urdu,
...
Rules: never invent a department. If the complaint is not a civic issue (crime, politics, billing dispute), use category "other" with low confidence.`;
